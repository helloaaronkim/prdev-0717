...
# Creates an asset for the student files of the devint-course (APDevPRInts) based on the current state of this repo.
...
# APDevPRInts4.3_studentFiles_07may2020.zip
#
# Just packages source code and can therefore be run from a fresh clone of this GitHub project.
#
# Usage: create-student-files-asset-for-devint.sh [secure-props-key]
...
assetname="$course2""_studentFiles_$today" # APDevPRInts_studentFiles_07may2020
assetfile="$assetname.zip"                 # APDevPRInts_studentFiles_07may2020.zip
...
# remove dir hieararchy for devprd
rm -rf "$assetname/$wtsreldir/devprd"
# overwrite pom.xml in walkthroughs dir to eliminate reference to devprd dir
grep -v 'devprd' "$solutionsdir/$wtsreldir/pom.xml" > "$assetname/$wtsreldir/pom.xml"
