...
# Creates an asset for the split student manual of the devprd-course (APDevPRDev) based on the current state of this repo.
...
# APDevPRDev4.3_studentManual_split_07may2020.zip
#
...
#
# Usage: create-split-student-manual-asset-for-devprd.sh <version>
...
assetname="$course1""_studentManual_split_$today" # APDevPRDev_studentManual_split_07may2020
assetfile="$assetname.zip"                        # APDevPRDev_studentManual_split_07may2020.zip

echo "Staging $assetname"
cd $assetsdir
rm -rf $assetname && mkdir $assetname
cd $assetname
cp "$manual1" ./manual.pdf                                    # copy manual to split
...
done < "$scriptdir/manual-mappings-devprd.csv"
